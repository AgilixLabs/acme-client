...
impl Account {
    ...
    pub async fn from_credentials(credentials: AccountCredentials) -> Result<Self, Error> {
        Ok(Self {
            inner: Arc::new(AccountInner::from_credentials(credentials).await?),
        })
    }
    ...
    pub async fn from_parts(
        ...
        Ok(Self {
            inner: Arc::new(AccountInner {
                ...
                client: Client::new(directory_url).await?,
            }),
        })
    }
...
impl AccountInner {
    async fn from_credentials(credentials: AccountCredentials) -> Result<Self, Error> {
        Ok(Self {
            id: credentials.id,
            key: Key::from_pkcs8_der(credentials.key_pkcs8.as_ref())?,
            client: match (credentials.directory, credentials.urls) {
                (Some(server_url), _) => Client::new(&server_url).await?,
...
impl Client {
    async fn new(server_url: &str) -> Result<Self, Error> {
        ...
        let rsp = http.get(server_url).send().await?;
